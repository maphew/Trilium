...
- **Throughput is measured, not guessed.** Every save logs one `[Save]` line (total, MB/s, encode vs
  transport-wait split, transport used) and the worker logs a `[Backup]` line for backups (reads vs
  waiting-on-consumer vs hashing/crypto). Read those before optimizing: the remaining backup-side
  cost on a phone is the container's incremental SHA-256, which is `@noble/hashes` pure JS
  (`packages/trilium-backup-container/src/backend-web.ts`) because WebCrypto has no streaming digest.
