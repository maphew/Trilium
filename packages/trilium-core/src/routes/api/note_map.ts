...
import { escapeHtml } from "../../services/utils/index";
...
/**
 * The llmChat counterpart of {@link findExcerpts}: quotes the assistant prose around each
 * `[[noteId]]` wiki-link pointing at the referenced note. Only assistant text blocks are
 * scanned — they are what creates a chat's internalLink relations in the first place (see
 * `findLlmChatLinks` in services/notes.ts) — so a chat that references the note purely
 * through tool calls yields no excerpts.
 */
...
function getBacklinks(req: Request<{ noteId: string }>): BacklinksResponse {
    ...
    return getFilteredBacklinks(note).map((backlink) => {
        ...
        const excerpts = sourceNote.type === "llmChat"
            ...
            : findExcerpts(sourceNote, noteId);
