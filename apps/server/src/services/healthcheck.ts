...
export interface HealthcheckTarget {
    /** True when Trilium terminates TLS itself, so the probe has to speak it too. */
    https: boolean;
 ...
 */
export function probeHealth({ https: overTls, port, host }: HealthcheckTarget): Promise<number> {
    return new Promise((resolve) => {
        const transport = overTls ? https : http;
        const scheme = overTls ? "https" : "http";
        const options: https.RequestOptions = {
            timeout: TIMEOUT_MS,
            // The probe asks whether the server answers, not who it is. A loopback certificate is
            // usually self-signed and issued for a name this request does not use, so verifying it
            // would report a healthy server as dead.
        ...
        const request = port !== 0
            ? transport.request(`${scheme}://${host}:${port}${PROBE_PATH}`, options, onResponse)
            : transport.request({ ...options, socketPath: host, path: PROBE_PATH }, onResponse);
