...
/**
 * The main app window, which is the one carrying the local bridge — not an iframe like the
 * PDF.js viewer. Falls back to any window, and to null where there is none at all.
 */
async function findMainClient() {
    // @ts-expect-error - self.clients is valid in service worker context
    const all = await self.clients.matchAll({ type: "window", includeUncontrolled: true });

    const client = all.find((c: { url: string }) => {
        const url = new URL(c.url);
        // Main app is at root or index.html, not in /pdfjs/ or other iframe paths
        return !url.pathname.startsWith("/pdfjs/");
    }) || null;

    return client || all[0] || null;
}

async function forwardToClientLocalServer(request, _clientId) {
    const client = await findMainClient();
