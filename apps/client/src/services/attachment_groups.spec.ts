...
import { attachmentGroupForRole, partitionAttachmentsByGroup, SYSTEM_ATTACHMENT_ROLES } from "./attachment_groups.js";
...
describe("attachmentGroupForRole", () => {
    it("puts what the user placed on one side and what the app made on the other", () => {
        ...
        for (const role of SYSTEM_ATTACHMENT_ROLES) {
            expect(attachmentGroupForRole(role), role).toBe("system");
        }
    });
