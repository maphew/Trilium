 ...
 *
 * Positions are read as `offsetTop`, which neither a scroll nor a transform of a slide still
 * running moves, so neither is taken for a change of place.
 */
export function useFlip(ref: RefObject<HTMLElement>, { selector, disabled }: FlipOptions) {
    ...
    useLayoutEffect(() => {
        ...
        for (const child of container.querySelectorAll<HTMLElement>(selector)) {
            ...
            if (!disabled && previous !== undefined && Math.abs(previous - top) >= 1) {
                moved.push({ child, by: previous - top });
            }
        }
...
function slide(element: HTMLElement, offset: number) {
