...
// the test the props instead, `noteIdChanged` being the one the field's behaviour hangs off.
...
vi.mock("../react/NoteAutocomplete", async () => {
    ...
    return {
        default: (props: Record<string, unknown>) => {
            ...
            return h("input", { id: props.id as string | undefined });
        }
    };
});
...
describe("RelationValuesInput", () => {
    ...
    it("points a host's own label at the box", async () => {
        await mount({ values: [], onCommit: vi.fn(), inputId: "field-id" });
        expect(container.querySelector("input")?.id).toBe("field-id");
    });
});
