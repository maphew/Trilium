...
import { DEFAULT_COLOR_PALETTE } from "../react/ColorPicker";
import MindMapNodePanel, { getCommonValue, MIXED } from "./MindMapNodePanel";

const THEME_PALETTE = ["#dd7878", "#ea76cb", "#8839ef"];
...
/**
 * A stand-in for the Mind Elixir instance exposing only what the panel touches: the theme palette,
 * the live selection and `reshapeNode`.
 */
...
describe("MindMapNodePanel", () => {
    it("offers the theme palette for branches and the default one for text and background", () => {
        const nodes = [buildNode()];
        const { mind } = buildMind(nodes);

        const container = renderInto(<MindMapNodePanel mind={mind} nodes={nodes} />);

        expect(presetCells(section(container, 0))).toHaveLength(DEFAULT_COLOR_PALETTE.length);
        expect(presetCells(section(container, 1))).toHaveLength(DEFAULT_COLOR_PALETTE.length);
        expect(presetCells(section(container, 2))).toHaveLength(THEME_PALETTE.length);
    });

    it("shows the colors the selection already carries", () => {
        const nodes = [buildNode({
            style: { color: DEFAULT_COLOR_PALETTE[1], background: DEFAULT_COLOR_PALETTE[2] },
            branchColor: THEME_PALETTE[0]
        })];
        const { mind } = buildMind(nodes);
    ...
    it("selects nothing for a property the selected nodes disagree on", () => {
        const nodes = [
            buildNode({ id: "a", style: { color: DEFAULT_COLOR_PALETTE[0] } }),
            buildNode({ id: "b", style: { color: DEFAULT_COLOR_PALETTE[1] } })
        ];
        const { mind } = buildMind(nodes);
    ...
    it("patches every selected node, and blanks the property when a color is cleared", () => {
        ...
        expect(reshapeNode).toHaveBeenNthCalledWith(1, mind.currentNodes[0], { style: { color: DEFAULT_COLOR_PALETTE[3] } });
        expect(reshapeNode).toHaveBeenNthCalledWith(2, mind.currentNodes[1], { style: { color: DEFAULT_COLOR_PALETTE[3] } });
        ...
        expect(reshapeNode).toHaveBeenNthCalledWith(1, mind.currentNodes[0], { branchColor: THEME_PALETTE[1] });
    });
